...
router.post('/change',(req,res)=>{
	...
	pool.query(sql,[$price,$size],(err,result)=>{
		if(err)throw err;
		if (result.affectedRows>0){
			res.send("修改成功");
			return;
...
//修改轮播图片
router.post('/removebanner',(req,res)=>{
	var $name=req.body.name;
	var $size=req.body.size;
	var sql='select * from dq_shopclassimg where name=? and size=?';
	pool.query(sql,[$name,$size],(err,result)=>{
...
router.post('/addbanner',(req,res)=>{
	...
	pool.query(sql,[$name,$size],(err,result)=>{
		...
		if (result.affectedRows=0){
			...
			var sql="update dq_shopclassimg set other='banner' WHERE name=? and size=?";
			pool.query(sql,[$name,$size],(err,result)=>{
			if(err)throw err;
			if (result.affectedRows>0){
			res.send("修改成功");
...
//上传商品信息 
router.post("/addshopp",upload.array("myimg",8),(req,res)=>{
 ...
 //得到上传图片路径
 var url="http://qjkang.applinzi.com/img/shop/"+fRand+suff
